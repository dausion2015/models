...
path_obj_detec = os.path.abspath(os.path.join(path_res,'object_detection'))
sys.path.append(path_res)
sys.path.append(path_slim)
sys.path.append(path_nets)
sys.path.append(path_obj_detec)
print(sys.path)
import hashlib
import io
import logging
...
def create_tf_record(output_filename,
  ...
  Args:
    ...
    mask_type: 'numerical' or 'png'. 'png' is recommended because it leads to
      smaller file sizes.
  """
  writer = tf.python_io.TFRecordWriter(output_filename)#建立TFRecordWriter类的实例
  for idx, example in enumerate(examples):#循环文件名列表
    if idx % 100 == 0:
      logging.info('On image %d of %d', idx, len(examples))#打印进度
    xml_path = os.path.join(annotations_dir, 'xmls', example + '.xml')#拼接ｘｍｌ文件名
    # mask_path = os.path.join(annotations_dir, 'trimaps', example + '.png')
    # print(xml_path,'***************************************')
    if not os.path.exists(xml_path):#文件名如果不在抛出异常
      logging.warning('Could not find %s, ignoring example.', xml_path)
      continue
    
    with tf.gfile.GFile(xml_path, 'r') as fid:
      xml_str = fid.read()
    xml = etree.fromstring(xml_str) #用此命令解析成ｄｉｃｔ
...
def main(_):
  ...
  annotations_dir = os.path.join(data_dir, 'annotations')#/home/lucky/下载/w8-data
  #trainval是缺失文件，自己组织。文件内容是所有图片名称但没有ｊｐｇ
